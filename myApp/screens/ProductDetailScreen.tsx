...
function AddToCartSheet({
  ...
  return (
    <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}>
      ...
      <View style={[sheetStyles.container, { backgroundColor: currentTheme.card }]}>
        {/* Handle bar */}
        <View style={sheetStyles.handleBar} />

        {/* Product preview */}
        <View style={sheetStyles.productPreview}>
          <View style={[sheetStyles.previewImage, { backgroundColor: isDarkMode ? COLORS.bgCardLight : '#EAEAEA' }]}>
            {product.image ? (
              <Image source={product.image} style={{ width: 72, height: 72, borderRadius: 8 }} resizeMode="cover" />
            ) : (
              <Ionicons name="diamond-outline" size={32} color={currentTheme.textMuted} />
            )}
          </View>
          <View style={sheetStyles.previewInfo}>
            <Text style={[sheetStyles.previewName, { color: currentTheme.text }]} numberOfLines={2}>{product.name}</Text>
          ...
          <TouchableOpacity style={[sheetStyles.closeBtn, { backgroundColor: currentTheme.border }]} onPress={onClose}>
            <Ionicons name="close" size={20} color={currentTheme.text} />
          </TouchableOpacity>
        </View>

        <View style={[sheetStyles.divider, { backgroundColor: currentTheme.border }]} />

        {/* Size Selection */}
        <View style={sheetStyles.section}>
          <Text style={[sheetStyles.sectionLabel, { color: currentTheme.text }]}>Kích thước</Text>
          <View style={sheetStyles.sizeGrid}>
            {sizes.map((size) => (
              <TouchableOpacity
                key={size}
                style={[sheetStyles.sizeChip, { borderColor: currentTheme.border }, selectedSize === size && sheetStyles.sizeChipActive]}
                onPress={() => setSelectedSize(size)}>
                <Text style={[sheetStyles.sizeChipText, { color: currentTheme.textMuted }, selectedSize === size && sheetStyles.sizeChipTextActive]}>
                  {size}
                </Text>
              </TouchableOpacity>
            ))}
        ...
        <View style={sheetStyles.section}>
          <View style={sheetStyles.quantityRow}>
            <Text style={[sheetStyles.sectionLabel, { color: currentTheme.text }]}>Số lượng</Text>
            <View style={[sheetStyles.quantityControl, { backgroundColor: isDarkMode ? COLORS.bgCardLight : '#EAEAEA', borderColor: currentTheme.border }]}>
              <TouchableOpacity
                style={[sheetStyles.qtyBtn, { backgroundColor: currentTheme.border }, quantity <= 1 && sheetStyles.qtyBtnDisabled]}
                onPress={() => setQuantity(Math.max(1, quantity - 1))}
                disabled={quantity <= 1}>
                <Ionicons name="remove" size={18} color={quantity <= 1 ? currentTheme.textMuted : currentTheme.text} />
              </TouchableOpacity>
              <Text style={[sheetStyles.qtyText, { color: currentTheme.text }]}>{quantity}</Text>
              <TouchableOpacity style={[sheetStyles.qtyBtn, { backgroundColor: currentTheme.border }]} onPress={() => setQuantity(quantity + 1)}>
                <Ionicons name="add" size={18} color={currentTheme.text} />
              </TouchableOpacity>
            </View>
          </View>
        </View>
        ...
        <View style={sheetStyles.footer}>
          <View>
            <Text style={[sheetStyles.totalLabel, { color: currentTheme.textMuted }]}>Tổng cộng</Text>
...
export default function ProductDetailScreen() {
  ...
  const { products, addToCart, isInWishlist, toggleWishlist, isAuthenticated, user, theme } = useAppContext();
  const currentTheme = Theme[theme];
  const isDarkMode = theme === 'dark';
  ...
  if (!product) {
    return (
      <View style={[styles.container, { backgroundColor: currentTheme.background }]}>
        <Text style={{ color: currentTheme.text, textAlign: 'center', marginTop: 50 }}>Sản phẩm không tồn tại.</Text>
      </View>
    );
  }
    ...
    <View style={[styles.container, { backgroundColor: currentTheme.background }]}>
      ...
      <SafeAreaView style={styles.safeArea} edges={['top']}>
        <View style={styles.header}>
          <Animated.View style={[styles.headerBg, { backgroundColor: currentTheme.background, opacity: headerBgOpacity }]} />
          <TouchableOpacity style={styles.headerBtn} onPress={() => router.back()}>
            <Ionicons name="arrow-back" size={22} color={currentTheme.text} />
          </TouchableOpacity>
          <Animated.Text style={[styles.headerTitle, { color: currentTheme.text, opacity: headerBgOpacity }]}>
            {product.name}
          </Animated.Text>
          <View style={styles.headerRight}>
            <TouchableOpacity style={styles.headerBtn} onPress={handleToggleWishlist}>
              <Ionicons name={liked ? 'heart' : 'heart-outline'} size={22} color={liked ? COLORS.red : currentTheme.text} />
            </TouchableOpacity>
            <TouchableOpacity style={styles.headerBtn}>
              <Ionicons name="share-outline" size={22} color={currentTheme.text} />
            </TouchableOpacity>
          </View>
        </View>
      </SafeAreaView>
      ...
      >
        ...
        <View style={styles.imageSection}>
          <Animated.View style={[
            ...
            {product.image ? (
              ...
              <View style={[styles.imagePlaceholder, { backgroundColor: isDarkMode ? COLORS.bgCardLight : '#EAEAEA' }]}>
                <Ionicons name="diamond-outline" size={80} color={currentTheme.textMuted} />
              </View>
            )}
          </Animated.View>
          ...
          {/* Gradient overlay */}
          <LinearGradient
            colors={['transparent', 'rgba(0,0,0,0.15)', currentTheme.background]}
        ...
        <Animated.View style={[styles.infoSection, { opacity: fadeAnim1, transform: [{ translateY: slideAnim1 }] }]}>
          ...
          <Text style={[styles.productName, { color: currentTheme.text }]}>{product.name}</Text>

          <View style={styles.ratingRow}>
            <View style={styles.starsRow}>{renderStars(product.rating)}</View>
            <Text style={[styles.ratingText, { color: currentTheme.text }]}>{product.rating}</Text>
            <Text style={[styles.reviewCount, { color: currentTheme.textMuted }]}>({product.reviews} đánh giá)</Text>
          </View>
        ...
        <Animated.View style={[styles.descSection, { opacity: fadeAnim2, transform: [{ translateY: slideAnim2 }] }]}>
          <Text style={[styles.sectionLabel, { color: currentTheme.text }]}>Mô tả sản phẩm</Text>
          <Text style={[styles.descText, { color: currentTheme.textMuted }]}>
        ...
        <Animated.View style={[styles.benefitsRow, { backgroundColor: currentTheme.card, borderColor: currentTheme.border, opacity: fadeAnim3, transform: [{ translateY: slideAnim3 }] }]}>
          {[
            ...
            <View key={i} style={styles.benefitItem}>
              ...
              <Text style={[styles.benefitText, { color: currentTheme.textMuted }]}>{b.text}</Text>
            </View>
          ))}
        </Animated.View>
        ...
        <View style={styles.reviewsSection}>
          <View style={styles.sectionHeaderRow}>
            <Text style={[styles.sectionLabel, { color: currentTheme.text }]}>Đánh giá thực tế ({reviews.length})</Text>
          ...
          {loadingReviews ? (
            <Text style={{ color: currentTheme.textMuted, paddingHorizontal: SPACING.xl }}>Đang tải đánh giá...</Text>
          ) : reviews.length === 0 ? (
            <Text style={{ color: currentTheme.textMuted, paddingHorizontal: SPACING.xl }}>Chưa có đánh giá nào. Hãy là người đầu tiên!</Text>
          ) : (
            reviews.map((r, i) => (
              <View key={i} style={styles.reviewCard}>
                <View style={styles.reviewHeader}>
                  <View style={[styles.reviewAvatar, { backgroundColor: isDarkMode ? COLORS.bgCardLight : '#EAEAEA' }]}>
                    <Text style={[styles.reviewAvatarText, { color: currentTheme.text }]}>{r.userName.charAt(0).toUpperCase()}</Text>
                  </View>
                  <View style={styles.reviewUserInfo}>
                    <Text style={[styles.reviewUserName, { color: currentTheme.text }]}>{r.userName}</Text>
                ...
                <Text style={[styles.reviewComment, { color: currentTheme.textMuted }]}>{r.comment}</Text>
              </View>
            ))
          )}
        </View>
        ...
        {related.length > 0 && (
          <View style={styles.relatedSection}>
            <Text style={[styles.sectionLabel, { color: currentTheme.text }]}>Sản phẩm liên quan</Text>
            <Animated.ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.relatedList}>
              {related.map((p) => (
                <TouchableOpacity
                  key={p.id}
                  style={[styles.relatedCard, { backgroundColor: currentTheme.card, borderColor: currentTheme.border }]}
                  onPress={() => router.push(`/product/${p.id}` as any)}>
                  <View style={[styles.relatedImage, { backgroundColor: isDarkMode ? COLORS.bgCardLight : '#EAEAEA' }]}>
                    {p.image ? (
                  ...
                  <Text style={[styles.relatedName, { color: currentTheme.text }]} numberOfLines={2}>{p.name}</Text>
      ...
      {/* Bottom action bar */}
      <View style={[styles.bottomBar, { backgroundColor: currentTheme.card, borderColor: currentTheme.border }]}>
        ...
        <Animated.View style={{ flex: 1, transform: [{ scale: addAnim }] }}>
          <TouchableOpacity
            ...
            <Ionicons name="bag-add-outline" size={20} color={product.inStock ? COLORS.black : currentTheme.textMuted} />
        ...
        <TouchableOpacity
          style={[styles.buyNowBtn, { backgroundColor: isDarkMode ? COLORS.white : COLORS.black }]}
          onPress={handleBuyNowPress}
          disabled={!product.inStock}>
          <Text style={[styles.buyNowText, { color: isDarkMode ? COLORS.black : COLORS.white }]}>MUA NGAY</Text>
        </TouchableOpacity>
      </View>
      ...
      <Modal visible={showReviewModal} transparent animationType="slide" onRequestClose={() => setShowReviewModal(false)}>
        <View style={sheetStyles.overlay}>
          <View style={[sheetStyles.container, { backgroundColor: currentTheme.card, marginTop: 'auto', padding: SPACING.xl }]}>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: SPACING.xl }}>
              <Text style={[sheetStyles.sectionLabel, { color: currentTheme.text }]}>Viết Đánh Giá</Text>
              <TouchableOpacity onPress={() => setShowReviewModal(false)}>
                <Ionicons name="close" size={24} color={currentTheme.text} />
              </TouchableOpacity>
            </View>
            
            <Text style={{ color: currentTheme.text, marginBottom: SPACING.sm }}>Đánh giá của bạn:</Text>
            ...
            <Text style={{ color: currentTheme.text, marginBottom: SPACING.sm }}>Chi tiết đánh giá:</Text>
            <View style={{ backgroundColor: currentTheme.background, borderRadius: BORDER_RADIUS.md, borderWidth: 1, borderColor: currentTheme.border, padding: SPACING.md, marginBottom: SPACING.xl }}>
              <TextInput 
                style={{ color: currentTheme.text, minHeight: 100, textAlignVertical: 'top' }}
...
const sheetStyles = StyleSheet.create({
  ...
  previewName: { fontSize: FONT_SIZES.md, fontWeight: '500', marginBottom: SPACING.xs },
  ...
  divider: { height: 0.5, marginHorizontal: SPACING.xl },
  section: { padding: SPACING.xl },
  sectionLabel: { fontSize: FONT_SIZES.lg, fontWeight: '500', marginBottom: SPACING.md },
  ...
  sizeChipText: { fontSize: FONT_SIZES.md, fontWeight: '500' },
  ...
  qtyText: {
    fontSize: FONT_SIZES.xl, fontWeight: '700',
  ...
  totalLabel: { fontSize: FONT_SIZES.sm, marginBottom: 2 },
...
const styles = StyleSheet.create({
  ...
  headerTitle: { fontSize: FONT_SIZES.md, fontWeight: '500', flex: 1, textAlign: 'center', zIndex: 1 },
  ...
  productName: { fontSize: FONT_SIZES.xxl, fontWeight: '400', fontStyle: 'italic', lineHeight: 30, marginBottom: SPACING.md },
  ratingRow: { flexDirection: 'row', alignItems: 'center', gap: SPACING.sm, marginBottom: SPACING.lg },
  starsRow: { flexDirection: 'row', gap: 2 },
  ratingText: { fontSize: FONT_SIZES.md, fontWeight: '600' },
  reviewCount: { fontSize: FONT_SIZES.sm },
  priceRow: { flexDirection: 'row', alignItems: 'center', gap: SPACING.md },
  price: { fontSize: FONT_SIZES.xxxl, fontWeight: '700' },
  originalPrice: { fontSize: FONT_SIZES.lg, textDecorationLine: 'line-through' },
  ...
  descText: { fontSize: FONT_SIZES.md, lineHeight: 24 },
  ...
  benefitText: { fontSize: FONT_SIZES.xs, textAlign: 'center', lineHeight: 16 },
  ...
  reviewAvatar: { width: 40, height: 40, borderRadius: 20, justifyContent: 'center', alignItems: 'center', marginRight: SPACING.md },
  reviewAvatarText: { fontSize: FONT_SIZES.md, fontWeight: '700' },
  reviewUserInfo: { flex: 1 },
  reviewUserName: { fontSize: FONT_SIZES.sm, fontWeight: '500', marginBottom: 2 },
  reviewStars: { flexDirection: 'row' },
  reviewDate: { color: COLORS.textMuted, fontSize: FONT_SIZES.xs },
  reviewComment: { fontSize: FONT_SIZES.sm, lineHeight: 22 },
  ...
  relatedName: { fontSize: FONT_SIZES.sm, padding: SPACING.sm, lineHeight: 18 },
  ...
  buyNowText: { fontSize: FONT_SIZES.sm, fontWeight: '700', letterSpacing: 0.5 },
});
